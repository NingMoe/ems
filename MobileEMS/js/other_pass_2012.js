...
function ga_track(name, code, beforeSiteTrack, config) {
    ...
    if (/\.hujiang\.com$/i.test(location.hostname)) {
        ...
        setTimeout(function () {
            ...
            e.src = "http://pass.hujiang.com/uc/hujiang_track.html?url=" + encodeURIComponent(url) + "&host=" + location.hostname + "&ref=" + encodeURIComponent(ref) + "&title=" + encodeURIComponent(document.title) + "&hash=" + encodeURIComponent(hash) + "&r=" + new Date().getTime();
            e.height = "0";
            e.width = "0";
...
function ga_writeCookie(name, value, expireDays) {
	var now = new Date();
	var exdate = new Date(now.valueOf() + expireDays * 86400000);
	document.cookie = name + "=" + encodeURIComponent(value) + ((expireDays == null) ? "" : ";expires=" + exdate.toGMTString()) + ";path=/;domain=hujiang.com";
}
...
function ga_pass_reg_succ() {
	if (location.pathname == "/m/signup/") {
		_gaq.push(['_trackPageview', '/m/signup_succ/' + location.search]);
	}

	if (location.pathname == '/m/signup_m/') {
		_gaq.push(['_trackPageview', '/m/signup_m_succ/' + location.search]);
	}

	ga_track_reg_search_keyword();
}

if (/(^\/focus|active)/.test(location.pathname)) { 
	ga_track_reg_search_keyword();
}
...
//#赵昕 2014年3月10日16:51:51


//注册时候，注册方式的选择
function ga_reg_type_click(type) {
	type = type.toString();	
	ga_track_event('reg', 'reg_type_click', type);
}

//点击注册按钮
function ga_reg_click(type) {
	type = type.toString();	
	ga_track_event('reg', 'reg_click', type);
}

//完善信息时，点击完善or跳过
function ga_user_info_click(type) {
	type = type.toString();	
	ga_track_event('reg', 'mail_user_info_click', type);
}

//完善信息时，点击完善or跳过
function ga_user_info_click(type, source) {
  	type = type.toString();	
  	source = source.toString();	
  	ga_track_event('reg', source + '_user_info_click', type);
}
...
//ga_load_js("http://common.hjfile.cn/analytics/site/TrackEvent.js");