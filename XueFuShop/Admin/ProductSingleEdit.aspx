<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Admin/MasterPage.Master" CodeBehind="ProductSingleEdit.aspx.cs" Inherits="XueFuShop.Admin.ProductSingleEdit" %>
...
<div class="position"><img src="Style/Images/PositionIcon.png"  alt=""/>逐个编辑</div>
<ul class="search">
...
<table class="listTable" cellpadding="0" cellpadding="0">
    <tr class="listTableHead">
	    ...
	    <td style="width:8%<%//if (XueFuShop.Common.ShopConfig.ReadConfigInfo().ProductStorageType == (int)ProductStorageType.SelfStorageSystem){ %>;display:none<%//} %>"> 总库存数量</td>
	    <td style="width:8%">库存下限</td>
	    <td style="width:8%">库存上限</td> 	
	    <td style="width:5%">管理</td>           
    </tr>
    <%foreach(XueFuShop.Models.ProductInfo product in productList){ %>		
    <tr class="listTableMain" onmousemove="changeColor(this,'#FFFDD7')" onmouseout="changeColor(this,'#FFF')">
        ...
        <td <%//if (XueFuShop.Common.ShopConfig.ReadConfigInfo().ProductStorageType == (int)ProductStorageType.SelfStorageSystem){ %>style="display:none"<%//} %>><input id="TotalStorageCount<%=product.ID%>" value='<%=product.TotalStorageCount%>' style="width:40px" class="input" title="总库存数量"/></td>
