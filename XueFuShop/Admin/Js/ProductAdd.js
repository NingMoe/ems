...
//搜索题库
function searchProductQuestionBank(action) {
    var productAccessoryName = o(globalIDPrefix+"AccessoryProductName").value;
    var classID = o(globalIDPrefix+"AccessoryClassID").value;
    var id= getQueryString("ID");    
    var url = "ProductAjax.aspx?ControlName=CandidateAccessory&Action=SearchProductQuestionBank&CourseName=" + encodeURI(productAccessoryName) + "&ClassID=" + classID;
    Ajax.requestURL(url, dealSearchProductQuestionBank);
    alertMessage("正在搜索...", 1); 
}
function dealSearchProductQuestionBank(data) {
    closeAlertDiv();
    var obj = o("CandidateAccessoryBox");
    obj.removeChild(o(globalIDPrefix+"CandidateAccessory"));
    obj.innerHTML = data;
...
function checkProduct() {
    ...
    var testStartTime=$("#"+globalIDPrefix+"TestStartTime").val(),
    testEndTime=$("#"+globalIDPrefix+"TestEndTime").val(),
    paperScore=$("#"+globalIDPrefix+"PaperScore").val(),
    testTimeLength=$("#"+globalIDPrefix+"TestTimeLength").val(),
    questionsNum=$("#"+globalIDPrefix+"QuestionsNum").val(),
    lowScore=$("#"+globalIDPrefix+"LowScore").val()
    if(testStartTime != ''|| testEndTime != ''){
        if((testStartTime == '' || testEndTime == '') || (paperScore == '' || testTimeLength == '' || questionsNum == '' || lowScore == '')){
            alertMessage("考试参数必须补全");
            return false;
        }
    }
    if (paperScore != '' || testTimeLength != '' || questionsNum != '' || lowScore != '') {
        if(paperScore == '' || testTimeLength == '' || questionsNum == '' || lowScore == ''){
            alertMessage("考试参数必须补全");
            return false;
        }
...
function addProductPhoto(photo,name){
    ...
    if(productID==""){
        ...
        Ajax.requestURL("Ajax.aspx?Action=AddProductPhoto&ProductID="+productID+"&Name="+encodeURIComponent(name)+"&Photo="+photo,dealAddProductPhoto);
    }
}
