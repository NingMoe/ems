if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SocoShop_CompareSTR]') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].[SocoShop_CompareSTR]
GO
--2．比较两个字符串中包含的数据项是否有任意一个相同
CREATE FUNCTION [dbo].[SocoShop_CompareSTR](
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SocoShop_CompareBrand]') and xtype in (N'FN', N'IF', N'TF'))
drop function [dbo].[SocoShop_CompareBrand]
GO
--2．比较两个品牌字符串中包含的数据项是否有任意一个相同（0为全匹配）
CREATE FUNCTION [dbo].[SocoShop_CompareBrand](
@s1  varchar(8000),  --要比较的第一个字符串
@s2  varchar(8000),  --要比较的第二个字符串
@split varchar(10)    --数据分隔符
)RETURNS bit
AS
BEGIN
	if CHARINDEX(@split+''0''+@split,@split+@s1+@split)>0 or CHARINDEX(@split+''0''+@split,@split+@s2+@split)>0
		RETURN(1)
...
AS 
	...
	ELSE 
	begin
		if CHARINDEX(''order by'',@condition)>0
			set @condition= left(@condition,CHARINDEX(''order by'',@condition)-1)
...
AS
SELECT [ID],[Status] FROM SocoShop_User WHERE [UserName]=@loginName AND [UserPassword]=@loginPass And [Status]<>0
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_ReadUserGroupID]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'

CREATE procedure [dbo].[SocoShop_ReadUserGroupID]
@companyID nvarchar(500)
AS

EXEC (''declare @result varchar(1000)
SELECT @result=isnull(@result+'''','''','''''''')+str([GroupID]) FROM [SocoShop_User] Where [companyID] in (''+@companyID+'') and [Status]<>0 Group by [groupID]
select isnull(replace(@result,'''' '''',''''''''),''''0'''') as GroupID'')
...
AS
	SELECT [ID],[FatherID],[OrderID],[ClassName],[Keywords],[Description],[TaobaoID],[IsDownload] FROM SocoShop_ProductClass ORDER BY [OrderID] ASC,ID ASC
' 
END
...
@isDownload int
AS 	
	UPDATE SocoShop_ProductClass SET [FatherID]=@FatherID,[OrderID]=@orderID,[ClassName]=@ClassName,[Keywords]=@Keywords, [Description]=@Description,[IsDownload]=@isDownload WHERE [ID]=@id   
...
AS
	SELECT [ID],[Name],[Logo],[Url],[Description],[OrderID],[IsTop],[ProductCount] FROM SocoShop_ProductBrand ORDER BY [OrderID],[Name]
' 
END
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_ReadAttributeRecordList]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_ReadAttributeRecordList]
@attributeID nvarchar(100),
@productID nvarchar(1000)
AS
	IF @attributeID=''''
	EXEC(''SELECT [AttributeID],[ProductID],[Value] FROM [SocoShop_AttributeRecord] WHERE [ProductID] in (''+ @productID +'')'')
	else
	EXEC(''SELECT [AttributeID],[ProductID],[Value] FROM [SocoShop_AttributeRecord] WHERE [ProductID] in (''+ @productID +'') And [AttributeID] in (''+ @attributeID +'')'')
' 
END


IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_ReadAttributeRecordAllList]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_ReadAttributeRecordAllList]
AS
	SELECT [AttributeID],[ProductID],[Value] FROM [SocoShop_AttributeRecord]
' 
END
...
AS 
	SELECT * FROM
(SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE [ID]=@id) As Temp1,
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_CompanyIndexStatistics]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_CompanyIndexStatistics] 
@companyID int
AS

	SELECT count(1) As [UserCount],sum(case when [status]=0 then 1 else 0 end) As [DelUserCount],sum(case when [Status]=1 then 1 else 0 end) As [NoCheckUserCount],sum(case when [Status]=2 then 1 else 0 end) As [NormalUserCount],sum(case when [Status]=3 then 1 else 0 end) As [FrozenUserCount],sum(case when [Status]=4 then 1 else 0 end) As [FreeUserCount] FROM [MOSDB].[dbo].[SocoShop_User] where [Companyid]=@companyID
...
@editor nvarchar(50)
AS 
	INSERT INTO SocoShop_Product([Name],[Spelling],[Color],[FontStyle],[ProductNumber],[ClassID],[BrandID],[MarketPrice],[Weight],[SendPoint],[Photo],[Keywords],[Summary],[Introduction],[Remark],[IsSpecial],[IsNew],[IsHot],[IsSale],[IsTop],[Accessory],[RelationProduct],[RelationArticle],[ViewCount],[AllowComment],[CommentCount],[SumPoint],[PerPoint],[PhotoCount],[CollectCount],[TotalStorageCount],[OrderCount],[SendCount],[ImportActualStorageCount],[ImportVirtualStorageCount],[LowerCount],[UpperCount],[AttributeClassID],[StandardType],[AddDate],[CompanyID],[Sort],[CourseDate],[State],[CourseURL],[SalesToolURL],[Editor]) VALUES(@name,@spelling,@color,@fontStyle,@productNumber,@classID,@brandID,@marketPrice,@weight,@sendPoint,@photo,@keywords,@summary,@introduction,@remark,@isSpecial,@isNew,@isHot,@isSale,@isTop,@accessory,@relationProduct,@relationArticle,@viewCount,@allowComment,@commentCount,@sumPoint,@perPoint,@photoCount,@collectCount,@totalStorageCount,@orderCount,@sendCount,@importActualStorageCount,@importVirtualStorageCount,@lowerCount,@upperCount,@attributeClassID,@standardType,@addDate,@companyID,@sort,@courseDate,@state,@courseURL,@salesToolURL,@editor)
	SELECT @@identity
' 
END
...
@isDownload int
AS 
	INSERT INTO SocoShop_ProductClass([FatherID],[OrderID],[ClassName],[Keywords],[Description],[TaobaoID],[IsDownload]) VALUES(@fatherID,@orderID,@className,@keywords,@description,@taobaoID,@isDownload)	
		SELECT @@identity
' 
END
...
@parentID int
AS 
	INSERT INTO SocoShop_UserMessage([MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName],[Mobile],[ParentID]) VALUES(@messageClass,@title,@content,@userIP,@postDate,@isHandler,@adminReplyContent,@adminReplyDate,@userID,@userName,@mobile,@parentID)
	SELECT @@identity
' 
END
...
@isChecked int
AS 
	UPDATE SocoShop_UserMessage Set [IsHandler]=@isHandler,[AdminReplyContent]=@adminReplyContent,[AdminReplyDate]=@adminReplyDate,[IsChecked]=@isChecked WHERE [ID]=@id
' 
END
...
AS 
	IF @userID=0
		SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName],[Mobile],[IsChecked] FROM SocoShop_UserMessage WHERE [ID]=@id
	ELSE
		SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName],[Mobile],[IsChecked] FROM SocoShop_UserMessage WHERE [ID]=@id AND [UserID]=@userID
' 
END


IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_ReadSonUserMessage]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_ReadSonUserMessage]
@parentID int,
@userID int
AS 
	IF @userID=0
		SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName] FROM SocoShop_UserMessage WHERE [ParentID]=@parentID
	ELSE
		SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName] FROM SocoShop_UserMessage WHERE [ParentID]=@parentID AND [UserID]=@userID
' 
END
...
AS 
	IF @condition=''''
		SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName],[Mobile],[IsChecked] FROM SocoShop_UserMessage 
	ELSE
		EXEC(''SELECT [ID],[MessageClass],[Title],[Content],[UserIP],[PostDate],[IsHandler],[AdminReplyContent],[AdminReplyDate],[UserID],[UserName],[Mobile],[IsChecked] FROM SocoShop_UserMessage WHERE ''+ @condition)
...
@workingPostID int,
@studyPostID int,
@workingPostName nvarchar(50),
@department int,
@idCard nvarchar(50),
@postStartDate datetime,
@entryDate datetime
AS 
	UPDATE SocoShop_User Set [Sex]=@sex,[UserName]=@userName,[Introduce]=@introduce,[Photo]=@photo,[MSN]=@mSN,[QQ]=@qQ,[Tel]=@tel,[Mobile]=@mobile,[RegionID]=@regionID,[Address]=@address,[Birthday]=@birthday,[Status]=@status,[Email]=@email,[GroupID]=@groupID,[RealName]=@realName,[CompanyID]=@companyID,[WorkingPostID]=@workingPostID,[StudyPostID]=@studyPostID,[WorkingPostName]=@workingPostName,[Department]=@department,[IDCard]=@idCard,[PostStartDate]=@postStartDate,[EntryDate]=@entryDate WHERE [ID]=@id
...
@id int
AS 
	SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE [ID]=@id
' 
END
...
AS 
	IF @condition=''''
		SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User 
	ELSE
		EXEC(''SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE ''+ @condition)
...
@openID nvarchar(50)
AS 
		SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE [OpenID]=@openID
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_ReadUserByMobile]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_ReadUserByMobile]
@mobile nvarchar(50)
AS 
		SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE [Mobile]=@mobile
...
@userName nvarchar(50)
AS 
	SELECT [ID],[UserName],[UserPassword],[Email],[Sex],[Introduce],[Photo],[MSN],[QQ],[Tel],[Mobile],[RegionID],[Address],[Birthday],[RegisterIP],[RegisterDate],[LastLoginIP],[LastLoginDate],[LoginTimes],[SafeCode],[FindDate],[Status],[OpenID],[GroupID],[RealName],[CompanyID],[WorkingPostID],[StudyPostID],[WorkingPostName],[Department],[IDCard],[PostStartDate],[EntryDate] FROM SocoShop_User WHERE [UserName]=@userName
...
AS
	SELECT [ID],[Name],[Power],[AdminCount],[AddDate],[IP],[Note],[CompanyID],[State] FROM SocoShop_AdminGroup 
' 
END
...
@state int
AS 
	UPDATE SocoShop_AdminGroup Set [Name]=@name,[Power]=@power,[Note]=@note,[State]=@state WHERE [ID]=@id
' 
END
...
@state int
AS 
	INSERT INTO SocoShop_AdminGroup([Name],[Power],[AdminCount],[AddDate],[IP],[Note],[CompanyID],[State]) VALUES(@name,@power,@adminCount,@addDate,@iP,@note,@companyID,@state)
' 
END
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocoShop_AddTestSetting]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [dbo].[SocoShop_AddTestSetting]
@id int,
@courseId int,
@paperScore int,
@testQuestionsCount int,
@testTimeLength int,
@lowScore int,
